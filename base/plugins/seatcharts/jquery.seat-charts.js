...
(function($) {
	...
	$.fn.seatCharts = function (setup) {
		...
		fn.append($headerRow);
		//2015年11月27日 16:46:00
		var i=0;
		//do this for each map row
		$.each(settings.map, function(row, characters) {
		//2015年11月27日 16:58:36
			var flag = false;
			var reg= /^[_~]+$/;
			if (!reg.test(characters)) //判断是否符合正则表达式
			{
				i++;
				flag=true;
			}
			...
			if (settings.naming.left) {
				//2015年11月27日 16:59:24
				if(flag){
					$row.append(
						$('<div></div>')
							.addClass('seatCharts-cell seatCharts-space').text(i)
							//2015年11月27日 16:57:33空排不排序号
			...
			$.each(characters.match(/[a-z_~]{1}(\[[0-9a-z_]{0,}(,[0-9a-z_ ]+)?\])?/gi), function (column, characterParams) { 
				...
				*/
				$row.append(character != '_' ?
					//if the character is not an underscore (empty space)
					(function(naming,chara) {
						if(chara=='~'){
							return $('<div></div>').addClass('seatCharts-half-cell seatCharts-space');
						}
						//so users don't have to specify empty objects
						settings.seats[character] = character in settings.seats ? settings.seats[character] : {};
